...
$question_no = 1;
$paper = '';
$questions = explode("\n\r", file_get_contents("public/papers/IAS_2017.txt"));
...
/*
    ...
    */
    ...
    $d_starting_array = explode("Solution:", $c_starting_array[1]);    
    $opD= $d_starting_array[0];
   
    
    $ans= $d_starting_array[1];
//        $ans = preg_replace('/\s/', '', $ans);
    $justification= explode("Justification:", $ans);
            $ans = preg_replace('/\s/', '', $justification[0]);
            $justification=nl2br($justification[1]);
...
//    $ans = ucfirst(preg_replace('/\s/', '', $ans_array[1]));
    ...
    echo "Answer: ".$ans."<br>";
    echo "Justification: <br> ".$justification."<br>";
